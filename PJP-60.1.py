...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # NEW: Transform 33 – XOR with 0xBF to map BE ↔ 01
    # ------------------------------------------------------------------
    def transform_33(self, data: bytes) -> bytes:
        """Bijective XOR transform: BE -> 01, 01 -> BE"""
        if not data: return b''
        return bytes(b ^ 0xBF for b in data)

    def reverse_transform_33(self, data: bytes) -> bytes:
        return self.transform_33(data)   # XOR is self-inverse
    ...
    def _build_transform_maps(self):
        ...
        # --- OVERRIDE transform 33 with our new BE↔01 XOR ---
    ...
    # ------------------------------------------------------------------
    ZADEN_MAGIC = 0x01  # <--- CHANGED from 0x33 to 0x01
...
def main():
    ...
    print("         Zaden files use a single‑byte header: 0x01 (changed from 0x33).")
