...
# ------------------------------------------------------------------
# Optional backends: paq, zstandard
# ------------------------------------------------------------------
try:
    import paq
except ImportError:
...
# No mandatory installs; we only try to import above.

# ---------- Prompt 1: Quantum (optional) ----------
...
# No zstandard or paq prompts – they are optional.
...
class UnifiedCompressor:
    ...
    def _find_best_16bit_key(self, data: bytes, quantum_boost: bool = False, time_limit: float = 60.0) -> int:
        ...
        if not quantum_boost or not HAS_QISKIT:
            ...
            # Simplified quantum‑boosted search using random shuffle (same as before)
    ...
    # FLT 25‑27 (simplified but lossless)
    ...
    # Transforms 28‑30 with backend compression (now backend may be raw)
    ...
    def _paqjp_transform_28(self, data: bytes) -> bytes:
        # Simplified: use block size 1024, transform via FLT, then backend compress
        BLOCK_SIZE = 1024
        if not data:
        ...
        for block_idx in range(total_blocks):
            ...
            # Simple XOR (reversible) as transform
            transformed = bytearray(chunk)
            key = (block_idx * 2654435761) & 0xFFFFFFFF
            for i in range(len(transformed)):
                transformed[i] ^= (key >> (8 * (i % 4))) & 0xFF
    ...
    # Transforms 29 and 30 simplified similarly (or use raw identity)
    def _paqjp_transform_29(self, data: bytes) -> bytes:
        return self._paqjp_transform_28(data)  # reuse
    def _paqjp_reverse_29(self, data: bytes) -> bytes:
        return self._paqjp_reverse_28(data)
    ...
    # NEW TRANSFORM 58 – Pack uniform-case hex digits (0-F) into bytes
    def transform_58(self, data: bytes) -> bytes:
        if not data:
            return b'\x00'
        # Must be even length and consist entirely of hex digits with uniform case
        if len(data) % 2 != 0:
            return b'\x00' + data
        ...
        if len(text) != len(data):
            return b'\x00' + data
        # Check uniform case
        is_upper = all('0' <= c <= '9' or 'A' <= c <= 'F' for c in text)
        is_lower = all('0' <= c <= '9' or 'a' <= c <= 'f' for c in text)
        ...
        for i in range(0, len(text), 2):
            ...
            try:
                ...
                return b'\x00' + data  # shouldn't happen
            packed.append(val)
        flag = 0x01 if is_upper else 0x02
        return bytes([flag]) + bytes(packed)
    ...
    def reverse_transform_58(self, data: bytes) -> bytes:
        ...
        if flag == 1:
            ...
            raise TransformError("Transform 58 reverse: invalid flag")
        packed = data[1:]
        out = bytearray()
        hex_digits = "0123456789ABCDEF" if case == 'upper' else "0123456789abcdef"
        for b in packed:
            high = (b >> 4) & 0xF
            low = b & 0xF
            out.append(ord(hex_digits[high]))
            out.append(ord(hex_digits[low]))
        return bytes(out)

    # Dynamic 48-255 (XOR with seed) – skip 57, 58
    ...
    def _build_transform_maps(self):
        ...
        # Explicitly assign transform 58
    ...
    # ------------------------------------------------------------------
    # Compression backends (raw if no zstd/paq)
    # ------------------------------------------------------------------
