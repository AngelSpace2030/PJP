...
+ NEW Transform 33: per‑3‑byte affine (lossless, bijective)
============================================================================
"""
...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # NEW Transform 33 – per‑3‑byte affine (bijective, lossless)
    # ------------------------------------------------------------------
    def transform_33(self, data: bytes) -> bytes:
        """
        Process data in 3‑byte chunks.
        For each chunk (value in 0..2^24-1), compute:
            new_val = (val * A + B) mod 2^24
        where A = 2*block_idx + 1  (always odd => invertible)
              B = (block_idx*7 + 13) & 0xFFFFFF
        The first byte of output stores the pad length (0,1,2) to restore original length.
        """
        if not data:
            return b'\x00'  # pad_len = 0, no payload
        pad_len = (3 - len(data) % 3) % 3
        padded = data + b'\x00' * pad_len
        out = bytearray([pad_len])
        mod = 1 << 24
        for i in range(0, len(padded), 3):
            ...
            val = int.from_bytes(chunk, 'little')
            block_idx = i // 3
            A = (2 * block_idx + 1) & 0xFFFFFF  # ensure odd
            B = (block_idx * 7 + 13) & 0xFFFFFF
            new_val = (val * A + B) % mod
            out.extend(new_val.to_bytes(3, 'little'))
        return bytes(out)
    ...
    def reverse_transform_33(self, data: bytes) -> bytes:
        ...
        for i in range(0, len(payload), 3):
            ...
            val = int.from_bytes(chunk, 'little')
            block_idx = i // 3
            A = (2 * block_idx + 1) & 0xFFFFFF
            B = (block_idx * 7 + 13) & 0xFFFFFF
            invA = pow(A, -1, mod)   # Python 3.8+ supports modular inverse
            orig_val = ((val - B) * invA) % mod
            out.extend(orig_val.to_bytes(3, 'little'))
        if pad_len:
            out = out[:-pad_len]
        return bytes(out)
    ...
    # ------------------------------------------------------------------
    # Build transform maps (1..256) including 31, 32, and 33
    # ------------------------------------------------------------------
    ...
    # Exclude non‑bijective (1,14,22,23,24,25,26,27,31,32) – but 33 is safe
    # ------------------------------------------------------------------
    ...
    def full_self_test(self) -> bool:
        ...
        # Test Transform 33
        print("\nTesting Transform 33 (per‑3‑byte affine) on random data...")
        test33 = bytes(rng.randint(0, 255) for _ in range(100))
        enc33 = self.transform_33(test33)
        dec33 = self.reverse_transform_33(enc33)
        if dec33 != test33:
            print("  FAIL: transform 33 round‑trip mismatch")
            all_ok = False
        else:
            print("  PASS: transform 33 round‑trip OK")
...
def main():
    print(f"{PROGNAME} – 256 transforms + 2704 pairs + Base64 + 6‑bit text + Quantum + Transforms 28–30 + .docx transforms 31–32 + NEW Transform 33 (per‑3‑byte affine)")
