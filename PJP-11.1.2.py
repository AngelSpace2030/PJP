...
- Exhaustive self‑test for selected transforms
...
# ---------- NEW: Prompt 4: Maximum transforms to test ----------
try:
    max_transforms_input = input("Option 4: Maximum number of transforms to test (1-256, default 256): ").strip()
    if max_transforms_input == "":
        max_transforms = 256
    else:
        max_transforms = int(max_transforms_input)
        if max_transforms < 1:
            max_transforms = 1
        elif max_transforms > 256:
            max_transforms = 256
    print(f"Using up to {max_transforms} transforms.")
except ValueError:
    print("Invalid number. Using default 256 transforms.")
    max_transforms = 256

PROGNAME = "UnifiedPAQJP+PJP (Configurable Transforms)"
...
class UnifiedCompressor:
    ...
    def __init__(self, max_transforms=256):
        self.max_transforms = max_transforms   # NEW: store the limit
    ...
    # ------------------------------------------------------------------
    # NEW TRANSFORM 58 – Bit‑plane separation (divide by two)
    # ------------------------------------------------------------------
    def transform_58(self, data: bytes) -> bytes:
        """Split each byte into high 7 bits and low 1 bit, store separately."""
        if not data:
        ...
        for i, b in enumerate(data):
            highs[i] = b >> 1          # high 7 bits
            low_bits.append(b & 1)     # low 1 bit
        # Pack low bits into bytes (8 per byte, zero-padded at the end)
        low_bytes = bytearray()
        for i in range(0, n, 8):
            chunk = low_bits[i:i+8]
            val = 0
            for bit in chunk:
                val = (val << 1) | bit
            if len(chunk) < 8:
                val <<= (8 - len(chunk))
            low_bytes.append(val)
        # Header: original length (4 bytes) + high bytes + low bytes
        out = bytearray(struct.pack('>I', n))
        out.extend(highs)
        out.extend(low_bytes)
        return bytes(out)

    def reverse_transform_58(self, data: bytes) -> bytes:
        if len(data) < 4:
            raise TransformError("Transform 58 reverse: data too short")
        ...
        for byte in low_bytes:
            for i in range(7, -1, -1):
                low_bits.append((byte >> i) & 1)
        low_bits = low_bits[:n]   # remove padding bits
        out = bytearray(n)
        for i in range(n):
            out[i] = (highs[i] << 1) | low_bits[i]
        return bytes(out)

    # Dynamic 48-255 (XOR with seed) – skip 57, 58 now custom
    ...
    # ------------------------------------------------------------------
    # Compression method (single transforms only) – uses self.max_transforms
    # ------------------------------------------------------------------
    def compress_with_verification(self, data: bytes, time_limit: Optional[float] = None) -> bytes:
        ...
        for t in range(1, self.max_transforms + 1):   # <-- changed from 256
    ...
    # ------------------------------------------------------------------
    # Compression method with LZH backend – uses self.max_transforms
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Full self‑test for selected transforms
    # ------------------------------------------------------------------
    def full_self_test(self) -> bool:
        print("=" * 60)
        print(f"Unified PAQJP+PJP – Self‑Test (testing {self.max_transforms} transforms)")
        print("=" * 60)
        print("Testing all selected transforms on multiple byte values...")
        ...
        if all_ok:
            print("\n  All selected transforms passed on test bytes.")
        else:
...
# ------------------------------------------------------------
def main():
    print(f"{PROGNAME} – Unified compression with configurable transforms")
    print(f"Using up to {max_transforms} transforms.")
    c = UnifiedCompressor(max_transforms=max_transforms)

    print("=" * 58)
    print("Options:")
    print("  1) Compress (Fast)      – selected transforms + backend")
    print("  2) Decompress")
    print("  3) Full self‑test       – verify selected transforms")
    print("  0) Exit")
    print("=" * 58)
