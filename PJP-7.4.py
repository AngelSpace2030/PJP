...
class UnifiedCompressor:
    ...
    # 25 – Dynamic dictionary tokenizer (now with ibytes up to 8)
    ...
    def _dynamic_dict_tokenize(self, data: bytes) -> bytes:
        """Tokenize with the smallest possible index length (1..8 bytes)."""
        try:
            text = data.decode('utf-8')
        except:
            return b'\x00' + data   # pass-through marker
        ...
        # Choose the smallest ibytes that can hold the maximum index (num_entries-1)
        if num_entries <= 256:                   # 2^8
            ibytes = 1
        elif num_entries <= 65536:               # 2^16
            ibytes = 2
        elif num_entries <= 16777216:            # 2^24
            ibytes = 3
        elif num_entries <= 4294967296:          # 2^32
            ibytes = 4
        elif num_entries <= 1099511627776:       # 2^40
            ibytes = 5
        elif num_entries <= 281474976710656:     # 2^48
            ibytes = 6
        elif num_entries <= 72057594037927936:   # 2^56
            ibytes = 7
        else:                                    # up to 2^64
            ibytes = 8

        header = bytearray()
        header.append(ibytes)                   # 1 byte storing the length
        header += struct.pack('>I', num_entries)  # 4-byte big-endian count (limited to 2^32-1)
        ...
        for chunk in chunks:
            ...
            token_stream += idx.to_bytes(ibytes, 'big')
        return bytes(header) + bytes(token_stream)

    def _dynamic_dict_detokenize(self, data: bytes) -> Optional[bytes]:
        if not data: return b''
        if data[0] == 0: return data[1:]          # pass-through
        ibytes = data[0]
        if ibytes not in (1,2,3,4,5,6,7,8): return None
        pos = 1
        ...
        tokens = []
        while pos < len(data):
            if pos + ibytes > len(data): break
            idx = int.from_bytes(data[pos:pos+ibytes], 'big')
            pos += ibytes
