...
# -*- coding: utf-8 -*-
"""
Ultimate Hybrid Compressor – 65536 Transform Paths + All Pipelines
==================================================================
Merges:
 - 65535 ordered transform pairs + raw (65536 total indices)
 - 256 base transforms (RLE, XOR, rotation, Fermat, dynamic, quantum…)
 - LZ77 + Huffman (2 KB window)
 - Hybrid dictionary (static‑word, line, dynamic)
 - Zaden block optimization (magic 0x33)
 - Algorithm 36 (magic 0x36)
 - Quantum‑inspired byte substitutions (9 fast, 8‑qubit) if Qiskit installed

Menu:
 1) Fast – 256 singles, backend (Zstd/PAQ)
 2) Ultra – 65536 indices, backend
 3) LZ77+Huffman fast
 4) LZ77+Huffman ultra
 5) Hybrid dictionary + Ultra backend (static, line, dynamic, picks best)
 6) Zaden block optimization
 7) Algorithm 36
 8) Full self‑test (all 65536 indices, dictionaries, Zaden, Algo36)
 9) Best Overall – tries modes 1,2,3,4,5,6,7 and picks smallest
10) Extract (decompress) any file produced by the above
 0) Exit

Decompression auto‑detects magic bytes (0x33, 0x36, 0xFF, DICT, LINE) and falls
back to standard header encoding.

Requires: Python 3.7+
Optional: zstandard, paq, qiskit (auto‑install prompt)
"""

import math, random, decimal, hashlib, base64, heapq, struct, os, tempfile, time
import re, urllib.request, sys, subprocess, importlib
from datetime import datetime
from typing import Optional, List, Tuple, Dict, Callable, Any
from collections import Counter, defaultdict

# ---------- Optional imports ----------
def _try_import(pkg, install_msg=True):
    try:
        return importlib.import_module(pkg), True
    except ImportError:
        if install_msg: print(f"Optional package '{pkg}' not found.")
        return None, False

paq_mod, HAS_PAQ = _try_import("paq")
zstd_mod, HAS_ZSTD = _try_import("zstandard")
if HAS_ZSTD:
    zstd_cctx = zstd_mod.ZstdCompressor(level=22)
    zstd_dctx = zstd_mod.ZstdDecompressor()

# ---------- Quantum transforms ----------
USE_QUANTUM, HAS_QISKIT = False, False
qc = input("Enable quantum‑inspired transforms (requires Qiskit)? (y/n): ").strip().lower()
if qc == 'y':
    try:
        ...
        if _install('qiskit'):
            ...
            except: print("Install ok but import failed – quantum disabled.")
...
def download_and_merge_dictionaries():
    ...
    if os.path.exists(COMBINED_DICT):
        print(f"Combined dictionary already exists. Skipping download.")
        return True
    ...
    if success == 0:
        print("No dictionaries downloaded. Continuing without.")
        return False
    with open(COMBINED_DICT, 'w', encoding='utf-8') as f:
        for w in sorted(all_words): f.write(w + '\n')
    print(f"Merged {len(all_words)} words.")
    return True
...
_CONST_DIAPASON_ITER_CODE = [
    (2,0b10),(2,0b11),(3,0b010),(3,0b011),(4,0b0010),(4,0b0011),(5,0b00010),(5,0b00011),
    (6,0b000010),(6,0b000011),(7,0b0000010),(7,0b0000011),(8,0b00000010),(8,0b00000011),
...
def sha256_8bytes(data): return hashlib.sha256(data).digest()[:8]
...
class UltimateHybridCompressor:
    def __init__(self, repeat_count=100):
        ...
        self.sequences = self._build_pair_sequences()    # 65535 pairs (t1,t2)
        ...
        if USE_QUANTUM and HAS_QISKIT:
            self._precompute_quantum_byte_substitutions()  # 9 fast quantum subs
    ...
    def transform_22(self,d): return d   # placeholder (was checksum, now identity)
    ...
    # ---------- Transforms 41‑47 special (from PAQJP) ----------
    ...
    def _build_transform_maps(self):
        ...
        # 48‑255 dynamic (quantum 48‑56 if enabled)
    ...
    # ---------- Pair sequences – all ordered pairs except (256,256) ----------
    ...
    # ---------- Full self‑test (all 65536 indices, LZH, dict, Zaden, Algo36) ----------
    def full_self_test(self):
        print("="*60)
        print("UltimateHybridCompressor – FULL SELF‑TEST (65536 indices)")
        print("="*60)
...
# ---------- Menu ----------
def main():
    print("Ultimate Hybrid Compressor – 65536 transformation paths")
    ...
    while True:
        ...
        if ch==0: break
            ...
            if ch==1: compressed=c.compress_with_best(data, ultra=False)
                ...
                methods = {}
                for name, func in [
                    ("StaticDict", lambda: c._compress_static_dict(data)),
                    ("DynamicDict", lambda: c._compress_dynamic_dict(data)),
                    ("LineDict", lambda: c._compress_line_dict(data)),
                ...
                if methods:
                    compressed = min(methods.values(), key=len)
                    print(f"Chosen method: {min(methods, key=lambda k: len(methods[k]))}")
                else:
                    print("All dict methods failed, falling back to Ultra.")
                    compressed = c.compress_with_best(data, ultra=True)
