...
+ AI‑powered compression (transform 33 + neural backend)
+ Block‑wise compression (256‑byte blocks) for maximum algorithmic flexibility.
  Option 9 compresses each block independently with the best of Absolute, Zaden,
  and Algorithm 36, then concatenates the results.
  Command‑line: pjp.py compress <in> <out>   (uses 256‑byte blocks)
============================================================================
"""
...
# ---------- Constants ----------
BLOCK_SIZE = 256  # You can change this to any power of two
...
# ------------------------------------------------------------------
# 2. Ask about other optional compression backends (zstandard, paq, AI, etc.)
# ------------------------------------------------------------------
other_choice = input("Install other optional compression backends (zstandard, paq, mpmath, cython, python-docx, transformers)? (y/n): ").strip().lower()
if other_choice == 'y':
    for pkg in ['mpmath', 'zstandard', 'cython', 'paq', 'python-docx', 'transformers', 'torch']:
        try:
...
# ---------- AI backend using transformers + torch ----------
HAS_AI = False
AI_MODEL = None
AI_TOKENIZER = None
...
try:
    ...
    AI_MODEL = AutoModelForCausalLM.from_pretrained(model_name)
    AI_MODEL.eval()
    HAS_AI = True
    print(f"AI backend loaded: {model_name}")
except ImportError:
    AI_MODEL = None
    AI_TOKENIZER = None
    HAS_AI = False
    print("AI backend not available (transformers or torch missing).")
except Exception as e:
...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # Transform 33 – AI Neural Tokenizer (text‑only, uses Hugging Face tokenizer)
    # ------------------------------------------------------------------
    def transform_33(self, data: bytes) -> bytes:
        ...
        tokens = AI_TOKENIZER.encode(text, add_special_tokens=True)
        out = bytearray()
        out.append(0x01)
        out.extend(struct.pack('<I', len(tokens)))
        for tok in tokens:
            out.extend(struct.pack('<H', tok))
        return bytes(out)
    ...
    def reverse_transform_33(self, data: bytes) -> bytes:
        ...
        if len(data) < 5:
            return data
        num_tokens = struct.unpack('<I', data[1:5])[0]
        pos = 5
        tokens = []
        for _ in range(num_tokens):
            ...
            tokens.append(tok)
        try:
            text = AI_TOKENIZER.decode(tokens, skip_special_tokens=True)
            return text.encode('utf-8')
        except Exception:
            return data
    ...
    # ------------------------------------------------------------------
    # Build transform maps (1..256) including 31, 32, 33
    # ------------------------------------------------------------------
    def _build_transform_maps(self):
        ...
        # 33 – AI Neural Tokenizer (text‑only, non‑bijective)
    ...
    def compress_with_best(self, data: bytes, ultra: bool = True,
        ...
        if not data:
            ...
            return compressed   # no need to verify because backend is always safe
        ...
        # Verify decompression – should always succeed now
        decomp, _ = self._decompress_auto(best_bytes)
        if decomp != data:
            # Fallback: store raw with marker (should never happen, but keep for safety)
    ...
    def _block_optimize(self, data: bytes, block_size: int = BLOCK_SIZE,
                        quantum_boost: bool = False, time_limit: float = 60.0) -> Tuple[bytes, List[Tuple[int, int]]]:
        keys = []
        transformed_parts = []
    ...
    # ------------------------------------------------------------------
    # New: Block‑wise compression for Option 9
    # ------------------------------------------------------------------
    def compress_blocks(self, data: bytes, block_size: int = BLOCK_SIZE,
                        time_limit: float = 300.0) -> bytes:
        """
        Split data into blocks of `block_size` bytes. For each block, try
        Absolute, Zaden, and Algorithm 36, pick the smallest, and store with
        a short header.
        """
        blocks = [data[i:i+block_size] for i in range(0, len(data), block_size)]
        out = bytearray()
        # Header: magic 0x37, block_size (2 bytes LE), number of blocks (2 bytes LE)
        out.append(0x37)
        ...
        for idx, block in enumerate(blocks):
            ...
            zaden_full += struct.pack('<II', block_size, len(zaden_keys))
            zaden_full += b''.join(self._encode_key_unary(k1) + self._encode_key_unary(k2) for k1, k2 in zaden_keys)
            zaden_full += zaden_compressed
            # Try Algorithm 36
            algo36_bytes = self.algorithm_36_compress(block, time_limit=time_limit)
            # Pick the smallest
            best = min(abs_bytes, zaden_full, algo36_bytes, key=len)
            # Store: method byte (0=Absolute, 1=Zaden, 2=Algorithm36) + length (2 bytes LE) + data
            if best is abs_bytes:
                method = 0
            elif best is zaden_full:
                method = 1
            else:
                method = 2
            out.append(method)
            out.extend(struct.pack('<H', len(best)))
            out.extend(best)   # now best is always bytes
            # Progress
            if (idx+1) % 10 == 0 or idx == len(blocks)-1:
                print(f"Block {idx+1}/{len(blocks)} compressed to {len(best)} bytes")
        return bytes(out)

    def decompress_blocks(self, data: bytes) -> Optional[bytes]:
        if len(data) < 1 or data[0] != 0x37:
            return None
        pos = 1
        if len(data) < pos + 4:
            return None
        block_size = struct.unpack('<H', data[pos:pos+2])[0]
        pos += 2
        num_blocks = struct.unpack('<H', data[pos:pos+2])[0]
        pos += 2
        out = bytearray()
        for _ in range(num_blocks):
            ...
            if pos + 2 > len(data):
                break
            blen = struct.unpack('<H', data[pos:pos+2])[0]
            pos += 2
            if pos + blen > len(data):
                break
            block_compressed = data[pos:pos+blen]
            pos += blen
            if method == 0:
                # Absolute: try hybrid decompression
                if block_compressed.startswith(self.MAGIC_LINE):
                    block = self._decompress_line_dict(block_compressed)
                elif block_compressed.startswith(self.MAGIC_DICT + b'\x01'):
                    block = self._decompress_static_dict(block_compressed)
                elif block_compressed.startswith(self.MAGIC_DICT + b'\x02'):
                    block = self._decompress_dynamic_dict(block_compressed)
                else:
                    block, _ = self._decompress_auto(block_compressed)
            elif method == 1:
                block = self.decompress_block_optimized(block_compressed)
            else:
                block = self.algorithm_36_decompress(block_compressed)
            if block is None:
                return None
            out.extend(block)
        return bytes(out)
    ...
    # ------------------------------------------------------------------
    # Enhanced Option 9: block‑wise compression with Absolute, Zaden, Algorithm36
    # ------------------------------------------------------------------
    def compress_with_best_plus_block(self, infile: str, outfile: str,
                                      block_size: int = BLOCK_SIZE,
                                      quantum_boost: bool = False,
                                      time_limit_per_block: float = 300.0):
        try:
        ...
        print(f"Block‑wise compression with block size {block_size} bytes...")
        start = time.time()
        result = self.compress_blocks(data, block_size, time_limit_per_block)
        ...
        print(f"Compressed {len(data)} → {len(result)} bytes (ratio: {len(result)/len(data):.2f}) in {elapsed:.2f}s → {outfile}")
    ...
    # ------------------------------------------------------------------
    # Decompression (handles standard PJP, Zaden, Algorithm36, and block mode)
    # ------------------------------------------------------------------
    def decompress_file(self, infile: str, outfile: str):
        ...
        # Check for block‑wise magic (0x37)
        if len(data) > 0 and data[0] == 0x37:
            original = self.decompress_blocks(data)
            if original is not None:
                with open(outfile, 'wb') as f:
                    f.write(original)
                print(f"Decompressed (block‑wise) → {outfile} ({len(original)} bytes)")
                return
            else:
                print("Decompression failed for block‑wise data.")
                return
    ...
    # ------------------------------------------------------------------
    # Full self‑test (exhaustive) – now includes block mode
    # ------------------------------------------------------------------
    def full_self_test(self) -> bool:
        ...
        # Test transform 33 (AI tokenizer) if AI is available
        if HAS_AI:
            print("\nTesting AI Neural Tokenizer (transform 33) on sample text...")
            sample_ai = b"This is a test of the AI tokenizer."
            enc33 = self.transform_33(sample_ai)
            dec33 = self.reverse_transform_33(enc33)
            if dec33 != sample_ai:
                print("  FAIL: AI tokenizer round‑trip mismatch")
                all_ok = False
            else:
                print("  PASS: AI tokenizer round‑trip OK")
        else:
            print("\n  SKIP: AI tokenizer not available.")
        ...
        # Block‑wise round‑trip test
        print("\nTesting block‑wise compression round‑trip...")
        test_block_data = bytes(rng.randint(0, 255) for _ in range(1024))
        compressed_blocks = self.compress_blocks(test_block_data, block_size=128, time_limit=1.0)
        decompressed_blocks = self.decompress_blocks(compressed_blocks)
        if decompressed_blocks != test_block_data:
            print("  FAIL: block‑wise round‑trip mismatch")
            all_ok = False
        else:
            print("  PASS: block‑wise round‑trip OK")
...
def main():
    if len(sys.argv) >= 4:
        ...
        if mode == "compress":
            c.compress_with_best_plus_block(infile, outfile, block_size=BLOCK_SIZE,
                                            quantum_boost=False, time_limit_per_block=300.0)
        elif mode == "algo36":
            try:
                with open(infile, 'rb') as f:
                    data = f.read()
                compressed = c.algorithm_36_compress(data, time_limit=300.0)
                with open(outfile, 'wb') as f:
                    f.write(compressed)
                print(f"Algorithm 36 compressed {len(data)} → {len(compressed)} bytes → {outfile}")
            except Exception as e:
                print(f"Error: {e}")
        else:
            print(f"Unknown mode: {mode}. Use 'compress' or 'algo36'.")
        return

    # Interactive menu
    print(f"{PROGNAME} – 256 transforms + 2704 pairs + Base64 + 6‑bit text + Quantum + Transforms 28–30 + .docx transforms 31–32")
    print("Option 9: block‑wise compression (256‑byte blocks) with Absolute, Zaden, and Algorithm 36; picks the smallest per block.")
    print("         Time limit per block can be set from 1 to 300 seconds.")
    print("         Block mode uses magic 0x37.")
    ...
    print("         AI‑powered compression (transform 33 + neural backend) is available if transformers/torch are installed.")
    print("Dictionary entries are read as plain text or Base64‑encoded UTF‑8.")
    if paq is None and not HAS_ZSTD and not HAS_AI:
    ...
    while True:
        ...
        print("5) Full self‑test (now includes Zaden, Algorithm 36, AI, and block mode)")
        ...
        print("9) Block‑wise compression (best per block, 256‑byte blocks)")
        print("0) Exit")
        print("="*50)
        ...
        if choice == 0:
            ...
            i = input("Input file: ").strip()
            o = input("Output file: ").strip() or i + ".pjp"
            c.compress_with_best_plus_block(i, o, block_size=BLOCK_SIZE,
                                            quantum_boost=False, time_limit_per_block=300.0)
