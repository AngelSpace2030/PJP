...
+ Zaden Block Optimization + Algorithm 36 (powers‑of‑two + smart candidates)
  Option 9: Algorithm 37 – AI Archive (compresses every 256‑byte block with a
            300‑second search per block, uses all 2704 transforms inside PJP,
...
class PJPCompressor:
    ...
    # Uses all 2704 transforms inside PJP (ultra=True) + PRNG XOR whitening
    # ------------------------------------------------------------------
    ALGO37_MAGIC = 0x37
    ...
    def algorithm_37_compress(self, data: bytes, block_size: int = 256, time_limit_per_block: float = 300.0) -> bytes:
        ...
        # ---- PRNG XOR to spread bytes 00‑FF ----
    ...
    # ------------------------------------------------------------------
    # Option 9 – Algorithm 37 compression (prompts for input/output)
    # ------------------------------------------------------------------
    def compress_ai_zaden(self):
        ...
        try:
            ...
            return
        print(f"Compressed {len(data)} → {len(compressed)} bytes (Algorithm 37) → {outfile}")
...
def main():
    ...
    print("Option 9: Algorithm 37 – AI Archive (all 2704 transforms, asks for input/output filenames)")
    ...
    while True:
        ...
        print("9) AI Archive – Algorithm 37 (all transforms, user‑specified I/O)")
        print("0) Exit")
        print("="*50)
