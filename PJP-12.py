...
+ Zaden Block Optimization + Algorithm 36 (powers‑of‑two + smart candidates)
  Option 9: Algorithm 37 – AI Archive that compresses every 256‑byte block
            with a 300‑second search per block, and saves the result
            with the name "AI_Zaden".
  (Uses PRNG XOR to spread bytes 00‑FF)
============================================================================
"""
...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # Algorithm 37 – AI Archive, 256‑byte blocks, 300s per block, output "AI_Zaden"
    # (Now with PRNG XOR to spread bytes 00‑FF)
    # ------------------------------------------------------------------
    ALGO37_MAGIC = 0x37
    ...
    def algorithm_37_compress(self, data: bytes, block_size: int = 256, time_limit_per_block: float = 300.0) -> bytes:
        ...
        # Compress with absolute PJP
        compressed_inner = self.compress_with_best(transformed_data, safe=False, ultra=True,
                                                   include_28=True, include_29=True, include_30=True)

        # ---- Replace constant XOR with PRNG XOR to spread bytes ----
        seed = random.randint(0, 65535)                 # 16‑bit seed
        rng = random.Random(seed)
        ...
        out += struct.pack('<H', seed)          # store 16‑bit seed
    ...
    # ------------------------------------------------------------------
    # Enhanced Option 9: Algorithm 37 – AI_Zaden
    # ------------------------------------------------------------------
    def compress_ai_zaden(self, infile: str):
        """Run Algorithm 37 compression and save to 'AI_Zaden'."""
        try:
            ...
            return
        compressed = self.algorithm_37_compress(data, block_size=256, time_limit_per_block=300.0)
        outfile = "AI_Zaden"
        try:
            ...
            return
        print(f"Compressed {len(data)} bytes → {len(compressed)} bytes (Algorithm 37) → {outfile}")
    ...
    def decompress_file(self, infile: str, outfile: str):
        ...
        # Check for Algorithm 37 magic (0x37)
    ...
    def full_self_test(self) -> bool:
        ...
        # Algorithm 37 round‑trip test
        print("\nTesting Algorithm 37 round‑trip...")
        test_algo37_data = bytes(rng.randint(0, 255) for _ in range(512))
        compressed_algo37 = self.algorithm_37_compress(test_algo37_data, block_size=256, time_limit_per_block=10.0)
        decompressed_algo37 = self.algorithm_37_decompress(compressed_algo37)
        if decompressed_algo37 != test_algo37_data:
            print("  FAIL: Algorithm 37 round‑trip mismatch")
            all_ok = False
        else:
            print("  PASS: Algorithm 37 round‑trip OK")
    ...
    # ------------------------------------------------------------------
    # Test 2704 pairs & extraction check – now includes Zaden and Algorithm 36,37
    # ------------------------------------------------------------------
    def test_2704_pairs_lossless(self) -> bool:
        ...
        # Algorithm 37 extraction test
        print("\nTesting Algorithm 37 extraction...")
        algo37_sample = b"Algorithm 37 test data. " * 30
        compressed_algo37 = self.algorithm_37_compress(algo37_sample, block_size=256, time_limit_per_block=10.0)
        decompressed_algo37 = self.algorithm_37_decompress(compressed_algo37)
        if decompressed_algo37 != algo37_sample:
            print("  FAIL: Algorithm 37 extraction mismatch")
            all_ok = False
        else:
            print("  PASS: Algorithm 37 extraction OK")
...
def main():
    ...
    print("Option 9: Algorithm 37 – AI Archive (compress every 256‑byte block with 300s search per block, saves as 'AI_Zaden')")
    ...
    while True:
        ...
        print("5) Full self‑test (now includes Zaden and Algorithm 36,37)")
        print("6) Decompress (extract)")
        print("7) Test 2704 pairs & extraction check (now includes Zaden and Algorithm 36,37)")
        print("8) Fast 256 transforms test (compress using 256 singles)")
        print("9) AI_Zaden compression (Algorithm 37)")
        print("0) Exit")
        print("="*50)
