...
+ Transforms 28–30 + .docx transforms 31–32 + Transform 37 (minus_37)
+ Zaden Block Optimization + Algorithm 36 (powers‑of‑two + smart candidates)
  ...
  Transform 37: for files ≤40 bytes, stores raw data with exactly 5 bytes overhead.
============================================================================
"""
...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # Transform 37 – minus_37: greedy power‑of‑two subtraction per 3‑byte chunk
    # Special small‑file mode: for input ≤40 bytes, store raw + 5 bytes overhead
    # ------------------------------------------------------------------
    def transform_37(self, data: bytes) -> bytes:
        """Greedy subtract largest power of two from each 3‑byte chunk (little‑endian).
           For files ≤40 bytes, stores raw data with a 5‑byte overhead to avoid expansion.
           Otherwise: 1-byte pad_len, then for each chunk: 2-byte count of exponents, then list of exponents (0‑23).
        """
        if not data:
            return b'\x00'

        # Small‑file optimisation: ≤40 bytes → raw storage + 5 bytes overhead
        if len(data) <= 40:
            out = bytearray()
            out.append(0xFF)                         # marker indicating small‑file mode
            out.extend(struct.pack('<H', len(data))) # little‑endian length
            out.extend(data)                         # raw bytes
            # pad with zeros so that total length = len(data) + 5
            pad_needed = (len(data) + 5) - len(out)
            out.extend(b'\x00' * pad_needed)
            return bytes(out)

        # Normal mode for >40 bytes
        pad_len = (3 - len(data) % 3) % 3
        padded = data + b'\x00' * pad_len
        out = bytearray([pad_len])
        for i in range(0, len(padded), 3):
            ...
            count = len(exponents)
            if count > 65535:
                raise ValueError("Unexpected large exponent count")
            out.extend(struct.pack('<H', count))
            out.extend(bytes(exponents))
        return bytes(out)

    def reverse_transform_37(self, data: bytes) -> bytes:
        if not data:
            return b''
        # Check for small‑file marker
        if data[0] == 0xFF:
            if len(data) < 5:
                return b''
            ...
            if length > 40:
                return b''  # invalid
            # raw data is stored at offset 3, length bytes, then padding ignored
        ...
        while pos + 2 <= len(payload):
            ...
            for i in range(count):
                ...
                val += (1 << exp)
                pos += 1
            out.extend(val.to_bytes(3, 'little'))
        if pad_len > 0:
            out = out[:-pad_len]
        return bytes(out)
    ...
    # ------------------------------------------------------------------
    # Build transform maps (1..256) including 31,32 and 37
    # ------------------------------------------------------------------
    def _build_transform_maps(self):
        ...
        # 37 – minus_37 (with small‑file optimised mode)
        self.fwd_transforms[37] = self.transform_37
        self.rev_transforms[37] = self.reverse_transform_37

        # 38‑255 dynamic
        for i in range(38, 256):
            fwd, rev = self._dynamic_transform(i)
            self.fwd_transforms[i] = fwd
            self.rev_transforms[i] = rev
    ...
    # Exclude non‑bijective (1,14,22,23,24,25,26,27,31,32,37)
    # ------------------------------------------------------------------
    def _build_pair_sequences(self) -> List[Tuple[int, int]]:
        safe = []
        for i in range(1, 257):
            if i in (1, 14, 22, 23, 24, 25, 26, 27, 31, 32, 37):
                continue
            safe.append(i)
            if len(safe) == 52:
                break
    ...
    def compress_with_best(self, data: bytes, safe: bool = False, ultra: bool = True,
            ...
            if not safe:
                ...
                if decomp != b'':
                    return self.compress_with_best(data, safe=True, ultra=ultra,
                                                   ...
                                                   include_30=include_30, include_37=include_37)
            return compressed
        ...
        if not include_37:
            single_transforms = [t for t in single_transforms if t != 37]
        ...
        if not include_37:
            allowed_pairs = [seq for seq in allowed_pairs if 37 not in seq]
    ...
    # ------------------------------------------------------------------
    def _compress_hybrid_bytes(self, data: bytes, include_37: bool = False) -> Tuple[bytes, str]:
    ...
    def algorithm_36_compress(self, data: bytes, time_limit: float = 300.0, include_37: bool = False) -> bytes:
        """
        Algorithm 36:
    ...
    def compress_with_best_plus_block(self, infile: str, outfile: str,
        ...
        print("Running Absolute compression (hybrid + Ultra + all transforms 28-30,37)...")
        abs_start = time.time()
        abs_bytes, abs_method = self._compress_hybrid_bytes(data, include_37=True)
        ...
        algo36_bytes = self.algorithm_36_compress(data, time_limit=300.0, include_37=True)
    ...
    def compress_file(self, infile: str, outfile: str, ultra: bool = True, hybrid: bool = False,
        ...
        if hybrid:
            best_bytes, method = self._compress_hybrid_bytes(data, include_37=False)
        else:
            candidates = []
            c_pjp = self.compress_with_best(data, safe=False, ultra=ultra,
                                            include_28=include_28, include_29=include_29,
                                            include_30=include_30, include_37=False)
    ...
    def full_self_test(self) -> bool:
        ...
        for mode_name, safe in [("marker‑free", False), ("safe", True)]:
            compressed = self.compress_with_best(test_data, safe=safe, ultra=True,
                                                 ...
                                                 include_30=True, include_37=True)
        ...
        # Test transform 37 (including small-file mode)
        print("\nTesting transform 37 (minus_37) on random data...")
        test37 = bytes(rng.randint(0, 255) for _ in range(100))
        enc37 = self.transform_37(test37)
        dec37 = self.reverse_transform_37(enc37)
        if dec37 != test37:
            print("  FAIL: transform 37 round‑trip mismatch")
            all_ok = False
        else:
            print("  PASS: transform 37 round‑trip OK")

        # Small-file mode for transform 37
        print("\nTesting transform 37 small-file mode (≤40 bytes)...")
        for size in [1, 15, 40, 41]:
            small_data = bytes(rng.randint(0, 255) for _ in range(size))
            enc_small = self.transform_37(small_data)
            dec_small = self.reverse_transform_37(enc_small)
            if dec_small != small_data:
                print(f"  FAIL: transform 37 small-file round‑trip for size {size}")
                all_ok = False
            else:
                overhead = len(enc_small) - len(small_data)
                print(f"  PASS: size {size} → overhead {overhead} bytes")
        if not all_ok:
            return False
        ...
        compressed_algo36 = self.algorithm_36_compress(test_algo36_data, time_limit=5.0, include_37=True)
    ...
    def test_2704_pairs_lossless(self) -> bool:
        ...
        try:
            ...
            self.compress_file(tmp_in_name, tmp_out_name, ultra=True, hybrid=True,
                               include_28=True, include_29=True, include_30=True)  # no include_37 in compress_file
        ...
        compressed_algo36 = self.algorithm_36_compress(algo36_sample, time_limit=5.0, include_37=True)
        decompressed_algo36 = self.algorithm_36_decompress(compressed_algo36)
...
def main():
    print(f"{PROGNAME} – 256 transforms + 2704 pairs + Base64 + 6‑bit text + Quantum + Transforms 28–30 + .docx transforms 31–32 + Transform 37 (minus_37)")
    print("Option 9: tries Absolute (hybrid + all transforms), Zaden block optimization, and Algorithm 36; picks the smallest.")
    print("         Transform 37 is *only* used in Option 9 (all three sub‑methods include it).")
    print("         For files ≤40 bytes, transform 37 stores data with exactly 5 bytes overhead.")
    ...
    while True:
        print("\n" + "="*50)
        print("Menu:")
        print("1) Fast (no 28-30,37) – 256 singles")
        print("2) Ultra (no 28-30,37) – 256 singles + 2704 pairs")
        print("3) Hybrid (no 28-30,37) – dicts + Ultra")
        print("4) Absolute (with 28,29,30 but NO 37) – all transforms except 37")
        print("5) Full self‑test (includes transform 37)")
        print("6) Decompress (extract)")
        print("7) Test 2704 pairs & extraction check (includes transform 37)")
        print("8) Fast 256 transforms test (no 28-30,37)")
        print("9) Zaden + Absolute + Algorithm 36 compare (ALL include transform 37)")
        print("0) Exit")
        print("="*50)
