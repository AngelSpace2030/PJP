...
- **Enhanced dynamic dictionary for line‑oriented data (1‑8 B, 1‑255 B lines, many lines)**
"""
...
class UnifiedCompressor:
    ...
    # ------------------------------------------------------------------
    # Enhanced dynamic dictionary tokenizer (transform 25)
    # - Detects line‑oriented text and splits by lines.
    # - Uses compact 1‑byte indexing for ≤255 unique lines.
    # - Greatly reduces size for 1‑8 B and 1‑255 B lines, and many lines.
    # ------------------------------------------------------------------
    ...
    def _is_line_oriented(self, text: str) -> bool:
        """Heuristic: if newline count is high relative to length, treat as lines."""
        lines = text.split('\n')
        if len(lines) <= 1:
            return False
        # many short lines → line-oriented
        avg_len = sum(len(l) for l in lines) / len(lines)
        return avg_len <= 255  # most lines are ≤255 bytes
    ...
    def _dynamic_dict_tokenize(self, data: bytes, index_bytes: int = 3) -> bytes:
        ...
        # Automatically choose split level based on content
        if self._is_line_oriented(text):
            level = 'line'
        else:
            level = 'all'

        chunks = self._split_text_into_chunks(text, level)
        freq = Counter(chunks)
        ...
        # Compact indexing: use 1 byte if ≤255, else 2, else 3, else 8
        if num_entries <= 255:
            use_index_bytes = 1
        elif num_entries <= 65535:
            use_index_bytes = 2
        elif num_entries <= 16777215:
            use_index_bytes = 3
        else:
            use_index_bytes = 8

        header = bytearray()
        header.append(use_index_bytes)          # 1 byte for index size
        # number of entries encoded with variable bytes to save space
        if num_entries < 256:
            header.append(num_entries)          # 1-byte count
        else:
            header += struct.pack('>I', num_entries)  # 4-byte count
        for chunk in sorted_chunks:
            chunk_bytes = chunk.encode('utf-8')
            # store chunk length as 2 bytes (max 65535, enough for lines)
            header += struct.pack('>H', len(chunk_bytes))
            header += chunk_bytes

        token_stream = bytearray()
        for chunk in chunks:
            idx = chunk_to_idx[chunk]
            if use_index_bytes == 1:
                token_stream.append(idx)
            elif use_index_bytes == 2:
                token_stream += struct.pack('>H', idx)
            elif use_index_bytes == 3:
                token_stream += struct.pack('>I', idx)[1:4]   # 3 bytes
            else:
    ...
    def _dynamic_dict_detokenize(self, data: bytes) -> Optional[bytes]:
        if not data: return b''
        if data[0] == 0: return data[1:]   # passthrough
        index_bytes = data[0]
        if index_bytes not in (1, 2, 3, 8):
            return None
        pos = 1
        ...
        for _ in range(num_entries):
            if pos + 2 > len(data):
                return None
            chunk_len = struct.unpack('>H', data[pos:pos+2])[0]
            pos += 2
