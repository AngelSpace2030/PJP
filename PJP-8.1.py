...
# -*- coding: utf-8 -*-
"""
Mega Unified Compressor – All Algorithms
========================================
Combines:
  - All 256+ transforms (RLE, XOR, PI, FLT, arithmetic, nibble, etc.)
...
COMBINED_DICT_FILE = os.path.join(DICT_DIR, "dictionary_combined.txt")
DICT_FILES = [
...
def download_and_merge_dicts():
    if not os.path.exists(DICT_DIR):
        os.makedirs(DICT_DIR)
    if os.path.exists(COMBINED_DICT_FILE):
        print("Combined dictionary already exists. Skipping download.")
        return True
    ...
    for fname, url in zip(DICT_FILES, DICT_URLS):
        ...
        try:
            ...
            if b'<html' in content[:200].lower():
                print(f"  WARNING: {fname} appears to be HTML, skipping.")
                continue
            ...
            with open(local, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    all_words.add(w)
            success += 1
            print(f"  Downloaded {fname} ({os.path.getsize(local)} bytes)")
        except Exception as e:
            print(f"  ERROR downloading {fname}: {e}")
    if success == 0:
        print("No dictionaries downloaded; continuing without static dictionaries.")
        return False
    with open(COMBINED_DICT_FILE, 'w', encoding='utf-8') as f:
...
def xor_prime_hash(word: str) -> bytes:
    ...
    return (total ^ prime).to_bytes(8, 'big')
...
# Prefix‑free nibble code (Constant Diapason)
...
_CONST_DIAPASON_DEC = {}
for nibble, (l, b) in enumerate(_CONST_DIAPASON_CODE):
    _CONST_DIAPASON_DEC[(l, b)] = nibble

# PAQ state table (transf. 47)
STATE_TABLE = [
...
class MegaCompressor:
    def __init__(self):
        download_and_merge_dicts()
    ...
    def transform_13(self, d):
        ...
        count = 0
        while count < repeats:
            current_value = nearest_prime(current_value)
    ...
    def transform_23(self, data: bytes) -> bytes:  # SHA‑256 word tokenizer
    ...
    def transform_24(self, data: bytes) -> bytes:  # XOR‑prime word tokenizer
        if not data: return b'\x00\x00\x00\x00'
        try:
            text = data.decode('latin-1')
        except:
    ...
    def transform_27(self, data: bytes) -> bytes:  # 6‑bit text with safeguards
        try:
            text = data.decode('utf-8')
    ...
    def _const_diapason(self, data: bytes) -> bytes:
    ...
    def _const_diapason_rev(self, data: bytes) -> bytes:
        ...
        for _ in range(pass_count):
            ...
            while pos < nbits:
                ...
                for length in range(2, 10):
                    ...
                    key = (length, codeword)
                    if key in _CONST_DIAPASON_DEC:
                        decoded_nibbles.append(_CONST_DIAPASON_DEC[key])
                        pos += length
                        matched = True
                        break
    ...
    # ---------- Block run (34) ----------
    def _block_run(self, data: bytes) -> bytes:
        if not data: return b''
        MAX_LEN = 43
        bits = []
        i = 0
    ...
    def _block_run_rev(self, data: bytes) -> bytes:
        if not data: return b''
        bits = []
        for byte in data:
            for i in range(7, -1, -1):
    ...
    def _flt_30(self, data):
        ...
        for block_idx in range(total_blocks):
            ...
            compressed_block = self._compress_backend(chunk)
            out.extend(enc_n)
            L = len(compressed_block)
            out.append((L >> 8) & 0xFF)
            out.append(L & 0xFF)
    ...
    def _build_transform_maps(self):
        ...
        # 31-32 identity
        self.fwd_transforms[31]=lambda x:x; self.rev_transforms[31]=lambda x:x
        self.fwd_transforms[32]=lambda x:x; self.rev_transforms[32]=lambda x:x
        # 33 Constant Diapason
        self.fwd_transforms[33]=self._const_diapason; self.rev_transforms[33]=self._const_diapason_rev
        # 34 Block run
        self.fwd_transforms[34]=self._block_run; self.rev_transforms[34]=self._block_run_rev
        # 35-40 FLT
        self.fwd_transforms[35]=self._flt_25; self.rev_transforms[35]=self._flt_25_rev
        self.fwd_transforms[36]=self._flt_26; self.rev_transforms[36]=self._flt_26_rev
        self.fwd_transforms[37]=self._flt_27; self.rev_transforms[37]=self._flt_27_rev
        self.fwd_transforms[38]=self._flt_28; self.rev_transforms[38]=self._flt_28_rev
        self.fwd_transforms[39]=self._flt_29; self.rev_transforms[39]=self._flt_29_rev
        self.fwd_transforms[40]=self._flt_30; self.rev_transforms[40]=self._flt_30_rev
        # 41-47
    ...
    def _precompute_quantum_transforms(self):
        ...
        base = 257
        for idx, perm in enumerate(self.quantum_fast_perms):
            fwd, rev = self._make_substitution_transform(perm, 256)
            self.fwd_transforms[base+idx] = fwd; self.rev_transforms[base+idx] = rev
        base2 = base + len(self.quantum_fast_perms)
        for idx, perm in enumerate(self.quantum_ultra_perms):
    ...
    def _make_permutation_transform(self, perm, block_size):
    ...
    def _load_static_dict(self):
        if not os.path.exists(COMBINED_DICT_FILE):
            return [], {}
        words_set = set()
        try:
            with open(COMBINED_DICT_FILE, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    w = line.strip()
                    if w: words_set.add(w)
        except Exception as e:
            print(f"Warning: could not read static dictionary: {e}")
            return [], {}
    ...
    def _load_line_dict(self):
        ...
        try:
            ...
            print(f"Warning: could not read line dictionary: {e}")
            return [], {}
        if not lines:
            return [], {}
    ...
    def compress_with_best(self, data: bytes, ultra: bool = True) -> bytes:
        if not data:
            return self._encode_marker_raw() + self._compress_backend(b'')
        best_total = float('inf')
        best_bytes = None

        def try_candidate(header, payload):
            nonlocal best_total, best_bytes
            candidate = header + self._compress_backend(payload)
            if len(candidate) < best_total:
                if self._decode_verify(candidate) == data:
        ...
        for t in range(1, 257):
            try:
                trans = self.fwd_transforms[t](data)
                try_candidate(self._encode_marker_single(t), trans)
            except: pass
        if ultra:
            for t1, t2 in self.sequences:
                try:
                    trans = self.fwd_transforms[t1](data)
                    trans = self.fwd_transforms[t2](trans)
                    try_candidate(self._encode_marker_pair(t1, t2), trans)
                except: pass
    ...
    def compress_with_lzh(self, data: bytes, ultra: bool = True) -> bytes:
        ...
        def try_candidate(header, payload):
            nonlocal best_total, best_bytes
            lzh = self._encode_lzh(payload)
            candidate = header + b'\xFF' + lzh
            if self._decode_lzh_verify(candidate) == data and len(candidate) < best_total:
                best_total = len(candidate)
                best_bytes = candidate
        ...
        if best_bytes is None:
            raise RuntimeError("Cannot compress this file with LZH.")
        return best_bytes

    def compress_lzh_only(self, data: bytes) -> bytes:
        lzh = self._encode_lzh(data)
        return self._encode_marker_raw() + b'\xFF' + lzh

    def _decode_verify(self, data: bytes) -> Optional[bytes]:
        ...
        if not seq: return res
        return self._reverse_sequence(res, seq)

    def _decode_lzh_verify(self, data: bytes) -> Optional[bytes]:
        offset, seq = self._decode_header(data)
        if offset == 0: return None
    ...
    def compress_file(self, infile, outfile="", ultra=True, use_lzh=False, lzh_only=False):
        with open(infile, 'rb') as f:
            data = f.read()
        if lzh_only:
            compressed = self.compress_lzh_only(data)
            suffix = ".pjp.lzh"
        elif use_lzh:
            ...
            suffix = ".pjp"
        if not outfile:
            outfile = os.path.basename(infile) + suffix
    ...
    def decompress_file(self, infile, outfile=""):
        ...
        if offset < len(data) and data[offset] == 0xFF:
            ...
            if transformed is None:
                print("LZH decompression failed")
                return
            if seq:
                original = self._reverse_sequence(transformed, seq)
            else:
            ...
            res = self._decompress_backend(payload)
            if res is None:
                print("Backend decompression failed")
                return
            if seq:
                original = self._reverse_sequence(res, seq)
            else:
                original = res
        if not outfile:
            base = os.path.basename(infile)
            outfile = re.sub(r'\.pjp(\.lzh)?$', '', base)
        self._atomic_write(outfile, original)
        print(f"Decompressed → {outfile} ({len(original)} bytes)")

    # ---------- Self‑tests ----------
    def test_all_indices(self) -> bool:
        ...
        for index in range(65536):
            try:
                ...
                for t in reversed(seq):
                    restored = self.rev_transforms[t](restored)
                if restored != test_data:
                    print(f"  FAIL at index {index}, sequence {seq}")
                    return False
            except Exception as e:
                print(f"  EXCEPTION at index {index}: {e}")
                return False
            if index % 10000 == 0:
                print(f"  ... {index} indices OK")
        print("  All 65536 indices lossless.")
        return True
    ...
    def test_lzh_pipeline(self) -> bool:
        print("Testing LZH pipeline with random 1000‑byte data...")
        rng = random.Random(12345)
        test_data = bytes(rng.randint(0, 255) for _ in range(1000))
        try:
            compressed = self.compress_with_lzh(test_data, ultra=True)
            if compressed is None:
                print("  FAIL: compression returned None")
                return False
            original = self._decode_lzh_verify(compressed)
            if original != test_data:
                print("  FAIL: LZH round-trip mismatch")
                return False
            print("  LZH pipeline OK")
            return True
...
# ---------- Main menu ----------
def main():
    print("Mega Unified Compressor – All Algorithms")
    c = MegaCompressor()
    while True:
        print("\n1) Compress (Fast) – single transforms + backend")
        print("2) Compress (Ultra) – 65536 pairs + backend")
        print("3) Compress (LZH only) – LZ77+Huffman, no transforms")
        print("4) Compress (Best) – tries Ultra backend and Ultra LZH, picks smallest")
        print("5) Self‑test (all 65536 indices) – transform lossless check")
        print("6) Decompress")
        print("7) Self‑test (LZH pipeline)")
        print("0) Exit")
        choice = input("> ").strip()
        if choice == "1":
            ...
            infile = input("Input file: ").strip()
            c.compress_file(infile, lzh_only=True)
        elif choice == "4":
            infile = input("Input file: ").strip()
            with open(infile, 'rb') as f:
                data = f.read()
            ...
            best_size = float('inf')
            try:
                cand1 = c.compress_with_best(data, ultra=True)
                if len(cand1) < best_size:
                    best = cand1
                    best_size = len(cand1)
            except: pass
            try:
                cand2 = c.compress_with_lzh(data, ultra=True)
                if len(cand2) < best_size:
                    best = cand2
                    best_size = len(cand2)
            except: pass
            if best is None:
                ...
                outfile = os.path.basename(infile) + ".pjp"
                c._atomic_write(outfile, best)
                print(f"Compressed {len(data)} → {len(best)} bytes → {outfile}")
        elif choice == "5":
            c.test_all_indices()
        elif choice == "6":
